...
## DEC-011: Report length emerges from evidence, not a target page count

Status: ACCEPTED (planned, v1.5.0)
Date: 2026-06-09
Decision: The long-form research report's length is an output of the evidence, not a target. The report must completely and faithfully cover the gathered, credible evidence at the depth the sources support; it must never pad thin evidence to reach a length nor truncate rich evidence to stay under one. Research-paper *structure* (abstract, themed sections, in-text citations, conclusion, references) is mandatory and is what makes it read as academic, independent of length. ~5–25 pages is a soft sanity envelope used only to flag degenerate output. The eval target is an "evidence-coverage" metric (fraction of credible gathered evidence faithfully synthesized and cited) plus a supported-claims ratio — not page count.
Alternatives considered: a hard 5–25 page range (rejected — forces padding on thin topics and truncation on rich ones, and padding invites hallucination); a fixed word-count target (same failure mode).
Reason: Page count is a rendering artifact the model does not control; targeting it degrades quality. Tying length to evidence coverage keeps reports honest and self-scaling. The lever for "appropriately long" is upstream evidence depth/breadth (more sources, full-text extraction, v1.4.0 scholarly repositories), so a too-thin report signals "gather more evidence," not "write more words."
