...
class SettingsTests(unittest.TestCase):
    def test_defaults_match_env_template(self) -> None:
        ...
        self.assertEqual("deterministic", settings.search_provider)
        self.assertIsNone(settings.search_api_key)
        self.assertEqual("https://api.search.brave.com/res/v1/web/search", settings.search_api_url)
        self.assertEqual(20, settings.max_steps_per_job)
        self.assertEqual(125000, settings.max_tokens_per_job)
        self.assertEqual(0.15, settings.max_model_cost_per_job_usd)
