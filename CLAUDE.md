...
- Current checkpoint: v1.5.0 — Long-Form Research Report CORE SHIPPED + LIVE. Multi-pass synthesis (outline → concurrent per-section drafting → framing, split model tiers) emits a research-paper structure (abstract, themed sections with in-text [n] citations, conclusion, references) behind `LONGFORM_ENABLED=true`; frontend renders it (falls back to flat when no sections). Verified in prod: a 4-section paper in ~315s. Prod long-form env: LONGFORM_ENABLED=true, MAX_MODEL_COST_PER_JOB_USD=0.75, JOB_TIME_BUDGET_SECONDS=150, JOB_HARD_TIMEOUT_SECONDS=420. OPERATIONAL: the long-lived cloakserve browser degrades — restart `CloakBrowser` service if research hangs; periodic recycling is a TODO. Remaining v1.5.0: evidence-coverage eval metric. Then v1.3.0 (shareable PDF) which depends on this
- Earlier: v1.2.3 Async Jobs & Live Progress (POST returns instantly, client polls, live per-step progress, serialized browser, hard per-job timeout) — fixed mobile "Failed to fetch"; v1.2.2 robustness; v1.2.1 Ghost stealth Phase 1; v1.2.0 Evidence Quality
...
- Live validation: DONE — first configured live eval artifact committed (`evals/results/eval_results_20260609T012342Z.json`, avg quality 0.398); eval scoring is now discriminative (integrity/quality split, no longer a flat 1.0)
- Known limits: stealth not yet deployed (Phase 1 on branch, prod still vanilla until merge); residential proxy measurement-gated; evidence depth/corroboration remain quality levers
- Next stage: DEC-010 decision — residential proxy for the 3 IP/ASN-blocked sites (62.5% usable already without proxy), and/or v1.2.1 Phase 2 (in-process `launch_context_async`: per-source fingerprint via `?fingerprint=`, humanize)
- CRITICAL FINDING: deployed `cloakserve` launches VANILLA headless Chromium, not CloakBrowser stealth — `fingerprint_seed` is discarded (`del`), `PROXY_*` unused. Production reports return empty (0% confidence) because Cloudflare blocks the Railway datacenter-IP headless browser. Local live eval only worked from a residential IP.
- v1.2.1 Ghost Phase 1 DONE (local): cloakserve now launches CloakBrowser's patched binary (`CLOAKSERVE_STEALTH=1` default; `=0` for vanilla baseline), Dockerfile.cloak installs `cloakbrowser` + pre-downloads binary, UA changed `HeadlessChrome/130`→`Chrome/146`, executor drives stealth over CDP, `evals/blocked_rate.py` harness added (block_rate 0.0 on production-failing targets from residential IP). DONE + DEPLOYED on Railway (merge 47ac653): CloakBrowser service serves Chrome/146 (was HeadlessChrome); datacenter-IP `blocked_rate` (via `railway ssh -s ghost-researcher`) went ~100% → 0.375 / usable 0.625. Loop closed: production research run now returns 0.68-confidence report, 5 findings, 7 sources (was empty 0%). Phase 1 DONE. 3 still blocked = IP/ASN-driven → DEC-010 resolved: option 1 (no proxy now); option 3 (proxy-on-retry) specced as optional Phase 2. Phase 2 = in-process `launch_context_async` honoring `fingerprint_seed` (CloakBrowser's own `bin/cloakserve` supports per-connection `?fingerprint=` over CDP — a Phase 2 option). Proxy measurement-gated (DEC-009, DEC-010)
- Planned: v1.5.0 Long-Form Research Report — turn the report from a one-page skeleton into a research-paper-structured doc (abstract, themed sections, in-text citations, conclusion, references) via multi-pass synthesis. Length EMERGES FROM EVIDENCE, not a page count (DEC-011): never pad thin evidence, never truncate rich; ~5-25pp is a soft envelope only; eval target is evidence-coverage + supported-claims, not pages. Prerequisite for v1.3.0 PDF. Needs evidence-depth + higher token/cost budgets. Est. cost/report: ~$0.05-0.20 cheap-tier, ~$0.20-0.55 mid, ~$0.40-2.50 premium. Recommended split-tier: cheap (flash) for research loop + per-section drafting, premium (Sonnet-class) for outline + abstract/conclusion/coherence pass (~$0.30-0.60 for a long report vs $2+ all-premium); raise `MAX_MODEL_COST_PER_JOB_USD` cap to a configurable ~$0.75 for long jobs (keep WARN + hard cap + graceful termination)
- Planned/deferred: v1.3.0 Shareable Report Output (`/reports/[id]` permalink + `/reports` list, OG/Twitter preview, server-rendered PDF export, print CSS, share/download). Splits link sharing (social) from file export (email/save/print) per DEC-008. DEPENDS ON v1.5.0 — PDF must render the research-paper structure, not the skeleton. PDF layout expected to need visual iteration
- Current regression baseline: 112 backend tests pass, 5 live smoke tests skipped
- v1.2.2 Pipeline Robustness (implemented): navigate closes prior pages before each nav (bounds shared CloakBrowser to ~1 page — fixes the connect_over_cdp hang that needed manual cloakserve restarts); wall-clock `JOB_TIME_BUDGET_SECONDS` (default 240) stops planning and synthesizes partial evidence so heavy jobs return a report instead of hitting the 300s request timeout; nav timeout 10s→20s + goto errors caught as skippable. Deferred: single CDP connection reuse per job (latency only). Discovered via prod verification: a slow source 500'd the run (fixed), then accumulated pages hung connect_over_cdp (fixed). VERIFIED in production: the heavy data-center goal that 500'd/hung now returns a 0.6-confidence report (5 findings, 4 sources, 8 visited) in ~226s. Railway watch paths configured (railway.api.json / railway.cloak.json) so cloakserve stops rebuilding on api/docs-only changes — needs per-service Config-as-code path set in the dashboard
