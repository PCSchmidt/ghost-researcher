...
- Next priority: v1.2.1 Ghost — integrate CloakBrowser library (pip `cloakbrowser`, clone at `..\CloakBrowser`) as the real stealth layer. Phased: P1 swap cloakserve to patched binary, P2 in-process `launch_context_async` (honor `fingerprint_seed`, per-source proxy/geoip/humanize). Proxy is measurement-gated (DEC-009, DEC-010). This IS the top live report-quality blocker
