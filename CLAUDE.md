...
- Next stage: v1.2.1 Phase 1 deploy to Railway + datacenter-IP block-rate measurement, then Phase 2 (in-process per-source fingerprint)
...
- v1.2.1 Ghost Phase 1 DONE (local): cloakserve now launches CloakBrowser's patched binary (`CLOAKSERVE_STEALTH=1` default; `=0` for vanilla baseline), Dockerfile.cloak installs `cloakbrowser` + pre-downloads binary, UA changed `HeadlessChrome/130`→`Chrome/146`, executor drives stealth over CDP, `evals/blocked_rate.py` harness added (block_rate 0.0 on production-failing targets from residential IP). Linux container de-risk PASSED (Dockerfile.cloak built 5.04GB image, patched binary launches, /json/version → Chrome/146 spoofing Windows from Linux, executor navigates through it). REMAINING: merge + deploy to Railway + capture datacenter-IP before/after (decisive, block was IP-driven). Phase 2 = in-process `launch_context_async` honoring `fingerprint_seed` (CloakBrowser's own `bin/cloakserve` supports per-connection `?fingerprint=` over CDP — a Phase 2 option). Proxy measurement-gated (DEC-009, DEC-010)
