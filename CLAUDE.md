...
- v1.2.1 Ghost Phase 1 DONE (local): cloakserve now launches CloakBrowser's patched binary (`CLOAKSERVE_STEALTH=1` default; `=0` for vanilla baseline), Dockerfile.cloak installs `cloakbrowser` + pre-downloads binary, UA changed `HeadlessChrome/130`→`Chrome/146`, executor drives stealth over CDP, `evals/blocked_rate.py` harness added (block_rate 0.0 on production-failing targets from residential IP). DEPLOYED + MEASURED on Railway (merge 47ac653): CloakBrowser service serves Chrome/146 (was HeadlessChrome); datacenter-IP `blocked_rate` (via `railway ssh -s ghost-researcher`) went ~100% blocked → block_rate 0.375 / usable 0.625 (5/8 previously-blocked targets now usable). Phase 1 exit criteria MET. 3 still blocked (iea.org, bloomenergy, datacenterknowledge) = IP/ASN-driven → DEC-010 residential-proxy decision point. Phase 2 = in-process `launch_context_async` honoring `fingerprint_seed` (CloakBrowser's own `bin/cloakserve` supports per-connection `?fingerprint=` over CDP — a Phase 2 option). Proxy measurement-gated (DEC-009, DEC-010)
